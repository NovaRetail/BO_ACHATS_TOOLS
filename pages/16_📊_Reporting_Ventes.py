"""
16_📊_Reporting_Ventes.py — Module Reporting Ventes · SmartBuyer Hub
Vue réseau (Rayon) + Destructeurs/Performeurs (Article) + Marge Négative par Site.
Export Excel nommé "Reporting Ventes - AAAAMMJJ.xlsx" (date du jour), avec la
période extraite automatiquement de la dernière cellule de la 1ère colonne
de l'export PBI.
"""
...
# ============================================================
st.set_page_config(page_title="Reporting Ventes · SmartBuyer", page_icon="📊", layout="wide", initial_sidebar_state="expanded")
...
def extract_periode(perimetre_text):
    ...
    de l'export PBI (texte de filtre). Retourne un texte de repli si le pattern est absent."""
...
def metric(container, label, value, delta=None, delta_color="normal"):
    """Wrapper autour de st.metric — remplace les anciennes kpi-card HTML
    pour coller au style natif Streamlit du module Bascule XD."""
...
def build_excel_full(k, perf, fam, n_top, art_res, perimetre, mns=None, mns_detail=None,
                      art_full=None, seuil_ca=100_000, periode=None):
    ...
    ws["B2"] = periode if periode else "Voir export source"; ws["B2"].font = Font(name=ARIAL, size=10, color="FF0000FF", bold=True)
    if perimetre:
    ...
    if mns_detail is not None and not mns_detail.empty:
        ...
        per_txt = periode if periode else (perimetre.replace("\n", " ")[:150] if perimetre else "Voir export source")
...
def render_destructeurs_performeurs(art_scope, n_top, seuil_ca, show_rayon=True):
    ...
    st.markdown(f"<span class='badge badge-red' style='margin:14px 0 6px;'>A · Marge négative</span>", unsafe_allow_html=True)
    ...
    st.markdown(f"<span class='badge badge-red' style='margin:14px 0 6px;'>B · Dégradation du taux de marge (marge encore positive)</span>", unsafe_allow_html=True)
    ...
    st.markdown(f"<span class='badge badge-green' style='margin:14px 0 6px;'>C · Performeurs — gain de marge en valeur</span>", unsafe_allow_html=True)
    ...
    st.markdown(f"<span class='badge badge-green' style='margin:14px 0 6px;'>D · Plus forte croissance de CA</span>", unsafe_allow_html=True)
    ...
    st.markdown(f"<span class='badge badge-red' style='margin:14px 0 6px;'>E · Plus forte baisse de CA</span>", unsafe_allow_html=True)
    show_table(res['E_baisse_ca'], ['CA','CA N-1','Perte CA (FCFA)'],
               {'CA': fmt, 'CA N-1': fmt, 'Perte CA (FCFA)': fmt}, show_rayon=show_rayon)

    st.markdown(f"<span class='badge badge-green' style='margin:14px 0 6px;'>F · Plus forte hausse de quantité vendue</span>", unsafe_allow_html=True)
    ...
    st.markdown(f"<span class='badge badge-red' style='margin:14px 0 6px;'>G · Plus forte baisse de quantité vendue</span>", unsafe_allow_html=True)
...
# INTERFACE
# ============================================================
st.markdown("<div class='page-title'>📊 Reporting Ventes — Vue réseau PGC</div>", unsafe_allow_html=True)
st.markdown("<div class='page-caption'>Marge négative par site · Destructeurs & Performeurs par rayon · objectifs marge alignés Méti · une seule extraction à charger</div>", unsafe_allow_html=True)
...
<div style='margin-bottom:18px'>
  ...
  <div style='font-size:11px;color:#8E8E93;margin-top:1px'>Hub analytique · Équipe Achats</div>
    ...
    st.markdown("---")
    st.caption("SmartBuyer Hub · Module Reporting Ventes")

if up is None:
    st.markdown("""
<div class='alert-card alert-blue'>
  <strong>ℹ️ À quoi sert ce module ?</strong><br>
  Ce module prépare le reporting ventes hebdo réseau à partir d'un seul export PBI
  <strong>Rayon → Famille → Sous-Famille → Article</strong> (avec ou sans colonne Site).
  Un fichier à charger chaque semaine, dans la barre latérale.
</div>
""", unsafe_allow_html=True)
    ...
    with c1:
        ...
        st.markdown("""
<div class='card'>
  <div style='font-size:14px;font-weight:700;color:#1C1C1E;margin-bottom:8px'>📋 Dashboard COPIL</div>
  <div style='font-size:12px;color:#3A3A3C;line-height:1.5'>
    CA, marge, quantités vs N-1 · performance par rayon vs objectifs Méti · marge négative par site ·
    top familles en baisse de CA / casse / poids promo.
  </div>
</div>
<div class='card'>
  <div style='font-size:14px;font-weight:700;color:#1C1C1E;margin-bottom:8px'>💥 Destructeurs & Performeurs</div>
  <div style='font-size:12px;color:#3A3A3C;line-height:1.5'>
    Articles agrégés réseau (sans doublon multi-site) : marge négative, dégradation de taux, gain de marge,
    croissance/baisse de CA et de quantité — disponible pour le réseau entier et pour chaque rayon dans son propre onglet.
  </div>
</div>
""", unsafe_allow_html=True)

    with c2:
        st.markdown("<div class='section-label'>Objectifs marge par rayon (Méti)</div>", unsafe_allow_html=True)
        st.markdown(f"""
<div class='format-card format-hyper'>
  <span class='badge badge-hyper'>Épicerie</span>
  <div class='small-muted' style='margin-top:6px'>Objectif : {CIBLES_DEFAUT['EPICERIE']:.1f}%</div>
</div>
<div class='format-card format-market'>
  <span class='badge badge-market'>Boissons</span>
  <div class='small-muted' style='margin-top:6px'>Objectif : {CIBLES_DEFAUT['BOISSONS']:.1f}%</div>
</div>
<div class='format-card format-supeco'>
  <span class='badge badge-supeco'>Droguerie / Parfumerie Hygiène</span>
  <div class='small-muted' style='margin-top:6px'>Objectifs : {CIBLES_DEFAUT['DROGUERIE']:.1f}% / {CIBLES_DEFAUT['PARFUMERIE HYGIENE']:.1f}%</div>
</div>
""", unsafe_allow_html=True)
...
<div class='alert-card alert-green'>
  <strong>1.</strong> Charge l'export PBI dans la sidebar.<br>
  <strong>2.</strong> Ajuste si besoin le nombre de lignes par classement et le seuil CA mini.<br>
  <strong>3.</strong> Consulte le Dashboard COPIL et les Destructeurs & Performeurs, réseau ou par rayon.<br>
  <strong>4.</strong> Télécharge le récap complet en Excel.
</div>
""", unsafe_allow_html=True)
    ...
    cols_expected = [
        ("Rayon", "Niveau réseau — obligatoire"),
        ("Famille", "Sous-niveau de Rayon — obligatoire"),
        ("Sous Famille", "Sous-niveau de Famille — obligatoire"),
        ("Article", "Maille la plus fine — obligatoire"),
        ("CA / CA N-1", "Chiffre d'affaires période et N-1"),
        ("Marge / %Vs N-1.1", "Marge et évolution du taux vs N-1"),
        ("Qté Vente / Qté Vente N-1", "Quantités vendues période et N-1"),
        ("Site (optionnel)", "Active la vue Marge négative par site"),
    ]
...
with tab1:
    ...
    if k is None:
        ...
        c1.metric("CA", f"{fmt(k['ca'])} FCFA", f"{k['evol_ca']*100:+.1f}% vs N-1")
        c2.metric("Marge", f"{fmt(k['marge'])} FCFA")
        evo_tx = k['tx_marge'] - k['tx_marge_n1'] if pd.notna(k['tx_marge_n1']) else np.nan
        c3.metric("Taux de marge", fmt_pct(k['tx_marge'], 2), fmt_delta(evo_tx) if pd.notna(evo_tx) else None)
        evo_qte = (k['qte']/k['qte_n1']-1)*100 if k['qte_n1'] else np.nan
        c4.metric("Qté vendue", fmt(k['qte']), f"{evo_qte:+.1f}% vs N-1" if pd.notna(evo_qte) else None)
        pct_casse = k['casse']/k['ca']*100 if k['ca'] else np.nan
        c5.metric("Casse", f"{fmt(k['casse'])} FCFA", fmt_pct(pct_casse, 2) + " du CA", delta_color="off")
        ...
        if mns is not None:
            ...
            m1.metric("Marge négative réseau", f"{fmt(total_neg)} FCFA", f"{nb_art} lignes article × site", delta_color="off")
            if pire is not None:
                m2.metric("Site le plus touché", str(pire['Site']), f"{fmt(pire['Marge Négative (FCFA)'])} FCFA", delta_color="off")
                m3.metric("Poids max vs CA site", fmt_pct(abs(pire['% Marge Nég / CA Site']), 2), str(pire['Site']), delta_color="off")
            disp_mns = mns.copy()
        ...
        xls = build_excel_full(k, perf, fam, n_top, art_res_export, perimetre, mns, mns_detail,
                                art_full=art, seuil_ca=seuil_ca, periode=periode)
        today_str = _dt.date.today().strftime("%Y%m%d")
        export_filename = f"Reporting Ventes - {today_str}.xlsx"
        st.download_button(f"📥 Télécharger {export_filename}", xls,
                            file_name=export_filename,
                            mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet")
        st.caption(f"Période détectée : {periode}")
