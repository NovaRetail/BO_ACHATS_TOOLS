...
if __name__ == "__main__":
    chemin_entree = sys.argv[1] if len(sys.argv) > 1 else "export.xlsx"
    chemin_sortie = sys.argv[2] if len(sys.argv) > 2 else "revue_performance.xlsx"
    df = charger_donnees(chemin_entree)
    construire_excel(df, chemin_sortie)
    print(f"OK — {chemin_sortie} généré, {len(df)} lignes article en entrée.")
