...
.mod-card {
    ...
    border-left: 3px solid; height: 100%;
}
...
.mod-formula { font-size: 11px; font-family: monospace; background: #F9F9FB; padding: 3px 8px; border-radius: 6px; color: #8E8E93; display: inline-block; margin-bottom: 6px; }
...
# ── MODULES ──────────────────────────────────────────────────────────────────
# (page, icon, title, color, badge_cls, badge, desc, formula, action)
SECTIONS = [
    {
        ...
        "modules": [
            ...
            ("pages/10_📊_Perf_Hebdo.py",           "📊", "Perf Hebdo",      "#5E35B1", "b-purp",  "Performance",
             ...
             "Export PBI — 1 onglet Excel par rayon",
    ...
    },
    {
        ...
        "modules": [
            ("pages/03_📦_Detention_Top_CA.py",     "📦", "Détention Top CA","#B45309", "b-amber", "Stock",
             "Taux de détention GOLD / SILVER · flux IM / LO · permanents.",
             "Détenu = stock > 0 · Taux = Nb détenus / Nb refs",
...
for section in SECTIONS:
    ...
    rows = [section["modules"][i:i+3] for i in range(0, len(section["modules"]), 3)]
    for row in rows:
        cols = st.columns(3)
